...
async function main() {
    ...
    const nftinuContract = await utils.deployContract("NFTInu");
    const fakeNFT = await utils.deployContract("FakeNFT");
    console.log(`NFTInu Contract address:`, nftinuContract.address);
    console.log(`FakeNFT Contract address:`, fakeNFT.address);

    saveFrontendFiles({
        "NFTInu": nftinuContract,
        "FakeNFT": fakeNFT
    });
    ...
    await fakeNFT.setApprovalForAll(nftinuContract.address, true);

    for (let i = 0; i < 5; i++) {
        id = await fakeNFT.tokenOfOwnerByIndex(owner.address, i);
        await nftinuContract.listNFT(
            id, // tokenId
    ...
    events = await nftinuContract.queryFilter("ListNFT");
    const listingIds = [];
    ...
    const listings = await nftinuContract.viewAllListings();
    for (let i = 0; i < listings.length - 2; i++) {
        ...
        await nftinuContract.connect(account1).borrow(
            listing.id,
